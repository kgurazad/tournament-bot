...
var helpSections = {
    ...
    'c': {
        name: 'Create Room',
        value: 'This command can only be run by users with the Control Room role. Room names cannot have spaces in them, and must be less than 90 characters long. If you want to create multiple rooms at a time, separate the room names with spaces. \nExample bot-style usage: `.c Room-1 Room-2`\nExample NL-style usage: `.create Room-1 Room-2`'
    },
    'f': {
...
var init = async function (guild) {
	...
	if (role.name !== '@everyone' && role.name !== 'Tournament Bot') {
	    await role.delete();
	}
...
var help = function (channel, sections) {
    sections = sections || ['i', 'c', 'f', 'd', 'm', 'a', 'r', 't', 'e', 'h'];
    var helpMessage = {
...
var schedule = async function (guild, docID, sheetIndex) {
    ...
    for (var i = 2; i < 26; i++) { // cols 3 to 26
    ...
    var teams = {};
    for (var i = 2; i < 26; i++) {
	rounds[i] = sheet.getCell(i, 0).value;
	for (var j = 2; j < 26; j += 2) {
	...
	var roomSchedule = {
	    ...
	    description: 'Run the commands listed here before/after each round to get teams to the right place. You can simply copy/paste the commands from this schedule.',
            fields: []
	};
	var i = 0;
...
client.on('message', function (message) {
    ...
    } else if (message.content.indexOf('.c') === 0 && message.member.roles.highest.name === 'Control Room' && message.author.tag === 'Entropy#1397') {
	try {
	...
	try {
        ...
        try {
            var text = message.mentions.channels.array()[0];
	    var parent = text.parent;
	    var name = parent.name;
	    // if (parent.children.length === 2) {
		confirm(message, 'Are you sure you want to delete room "' + name + '"? Confirm by reacting with \:thumbsup:.', function () {
                    message.channel.send('No confirmation was received. The creation is cancelled.');
		}, function () {
                    deleteRoom(text).then(function () {
			message.channel.send('Room "' + name + '" has been deleted.');
                    }).catch(function (error) {
			console.error(error);
                        message.channel.send('Room "' + name + '" could not be deleted. Please delete it manually.')
...
*/
    ...
    } else if (message.content.indexOf('.s') === 0) {
	try {
	    var content = message.content.split(/\s+/g);
    ...
    } else if (message.content.indexOf('.m') === 0) {
	try {
	    var range = message.content.substr(message.content.indexOf(' ') + 1).trim();
